...
## 2024-07-28 - Intermediate Array Allocation in Unique Sets
**Learning:** Found several places where unique values were calculated using the pattern `Array.from(new Set([...defaults, ...array.map(item => item.value)])).filter(Boolean)`. The nested `.map()` array execution causes unnecessary garbage collection pressure by creating an intermediate array before immediately discarding it inside the `Set` constructor.
**Action:** When extracting unique values or iterating over large data arrays, prefer standard index-based `for` loops directly populating a `Set` initialized with any default values to prevent intermediate array allocation overhead.
