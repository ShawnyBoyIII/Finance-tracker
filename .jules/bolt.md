...
## 2024-08-04 - $O(A \times T)$ nested loops during render mapping
**Learning:** In `Dashboard.tsx`, the `cardSummaries` logic filtered through `transactions` array repeatedly *inside* a `.map` loop mapping over `accounts`. For each credit card account, it performed up to 4 separate `.filter` or `.reduce` operations over `transactions`. As transactions grow, this block blocks the main thread during render loops. Furthermore, extracting `new Date().toISOString().slice(0, 7)` in an inner loop runs repetitively.
**Action:** Replace $O(A \times T)$ nested arrays inside render loop `.map()` logic with single-pass $O(T)$ Hash Map pre-aggregation logic outside of the array map to ensure predictable scale as local datasets grow. Hoist repetitive date extractions out of loops.
