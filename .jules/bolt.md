...
**Learning:** Instantiating `Intl.NumberFormat` and `Intl.DateTimeFormat` inside frequently called utility functions (like `formatCurrency` or `getMonthLabel`) introduces measurable performance overhead during render cycles and tight loops.
**Action:** Always instantiate `Intl` formatter objects at the module level (outside the functions) to cache and reuse them, preventing repeated memory allocation and initialization costs.
