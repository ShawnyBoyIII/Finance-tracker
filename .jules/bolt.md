...
## 2024-05-19 - Avoid filter(Boolean).join() in hot loops
**Learning:** In Next.js/React `useMemo` hooks operating on large data sets (like `TransactionList`), chaining `.filter(Boolean).join(' ')` to create search haystacks dynamically inside a `.filter()` iteration is a significant performance bottleneck due to continuous intermediate array allocation and destruction.
**Action:** Replace dynamic array construction and `.filter(Boolean).join()` with direct string concatenation, using inline fallbacks like `(prop || '') + ' ' + (prop2 || '')` to dramatically reduce garbage collection pressure.
