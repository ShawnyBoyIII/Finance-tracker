...
\n## 2026-07-12 - O(N log N) Array mapping and sorting to find bounds\n**Learning:** In `CSVImport.tsx` and `PDFImport.tsx`, mapping over large arrays to extract dates and subsequently applying `.sort()` to extract the `min` and `max` dates introduces both unnecessary array allocation memory overhead and expensive (N \log N)$ execution time overhead. For large file imports, this significantly bogs down the main thread.\n**Action:** Use a single-pass (N)$ `for` loop to find both `min` and `max` variables simultaneously, completely eliminating intermediate array instantiation and the costly sort operation.
