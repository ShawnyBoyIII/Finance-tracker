...
## 2024-06-05 - Avoid .map().filter() and Set overheads in React Render Loops
**Learning:** Chaining `.map()` and `new Set()` inside a React render loop (e.g., `Array.from(new Set(transactions.map(t => t.category)))`) creates severe memory pressure by allocating an entire new array of categories, just to immediately throw it away. Additionally, using `.localeCompare` for sorting is significantly slower than standard comparison operators.
**Action:** Use a fast standard `for` loop to populate a `Set` directly, avoiding intermediate array allocations. However, remember that standard comparative operators (`<`, `>`) break alphabetical locale sorting, so `localeCompare` must be retained when sorting user-facing strings (like category names), while comparative operators are safe for strict ISO date strings.
