...
## 2026-06-11 - Confirmation Dialogs Extended to Planners
**Learning:** While `TransactionList` had a confirmation dialog for deletions, planners (`BillPlanner`, `SalaryPlanner`, `ElectricityTracker`) were executing delete actions immediately on click. Consistent safety checks are essential across all destructive actions in the app.
**Action:** Ensure that *all* features that involve deleting items from local storage wrap the action in a `window.confirm` dialog.
