...
## 2024-06-27 - Form and Action Accessibility in Internal Components
**Learning:** Found a pattern where internal components (like `BillPlanner`) have form inputs without explicit `id` and `htmlFor` label bindings, and action buttons lack explicit `focus-visible` styles, hindering screen readers and keyboard navigation. Nested checkbox labels without explicit associations were also observed.
**Action:** Always ensure that form inputs have distinct IDs that are explicitly bound to their corresponding labels using `htmlFor`, un-nest checkbox labels for clearer association, and apply `focus-visible` rings to interactive elements, particularly for custom secondary dashboards/managers.
