...
export default function Dashboard() {
  ...
  const cardSummaries = useMemo(() => {
    // ⚡ Bolt: Calculate current month prefix once outside the loops
    const currentMonthPrefix = new Date().toISOString().slice(0, 7);

    // ⚡ Bolt: Pre-aggregate charges and payments in a single O(N) pass
    const accountStats = new Map<string, { charges: number; payments: number }>();
    for (let i = 0; i < transactions.length; i++) {
      const transaction = transactions[i];
      if (transaction.accountId && transaction.date.startsWith(currentMonthPrefix)) {
        let stats = accountStats.get(transaction.accountId);
        if (!stats) {
          stats = { charges: 0, payments: 0 };
          accountStats.set(transaction.accountId, stats);
        }
    ...
    return accounts
      ...
      .map((account) => {
        // ⚡ Bolt: O(1) lookup instead of O(N) filter and reduce
        const stats = accountStats.get(account.id) || { charges: 0, payments: 0 };

        // ⚡ Bolt: Replace intermediate array allocation with a standard for loop
