...
export default function Dashboard() {
  ...
  // ⚡ Bolt: Use direct comparison instead of localeCompare for ISO dates
  ...
  const cardSummaries = useMemo(() => {
    // ⚡ Bolt: Hoisted date calculation and O(N) pre-aggregation to avoid O(N*M) nested filtering
    const currentMonthKey = new Date().toISOString().slice(0, 7);
    const accountStats = new Map<string, { charges: number; payments: number }>();

    for (let i = 0; i < transactions.length; i++) {
      const t = transactions[i];
      if (!t.accountId || t.date.slice(0, 7) !== currentMonthKey) continue;

      let stats = accountStats.get(t.accountId);
      if (!stats) {
        stats = { charges: 0, payments: 0 };
        accountStats.set(t.accountId, stats);
      }

      if (t.type === 'expense') {
        stats.charges += t.amount;
      } else if (t.type === 'cc_payment') {
        stats.payments += Math.abs(t.amount);
      }
    }
