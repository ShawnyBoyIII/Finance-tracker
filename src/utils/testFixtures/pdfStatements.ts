export const chaseStatementFixture = `
  Chase Freedom
  Statement Date: 01/02/2026
  Date of Transaction Merchant Name or Transaction Description $ Amount
  12/28 AUTOMATIC PAYMENT - THANK YOU -179.85
  12/04 TELLO US 866-3770294 GA 15.85
  12/20 MENERALS LLC 775-684-9000 NV 44.00
  12/21 TARGET.COM * WWW.TARGET.CO MN 4.24
  12/24 APEX SPIN AND FITNESS APEXSPINANDFI CA 5.00
  12/31 PUBLIX #803 DACULA GA 7.38
  Total fees charged in
`;

export const chaseNoisyStatementFixture = `
  Chase Freedom Unlimited
  Opening/Closing Date 12/03/2025 - 01/02/2026
  Statement Date: 01/02/2026
  Cardmember Services 1-800-000-0000
  Date of Transaction Merchant Name or Transaction Description $ Amount
  12/28 AUTOMATIC PAYMENT - THANK YOU -179.85
  12/29 AMAZON.COM*MKTP US AMZN.COM/BILL WA 33.17
  12/30 SPOTIFY USA NEW YORK NY 11.99
  12/31 PUBLIX #803 DACULA GA 7.38
  2026 Totals Year-to-Date
`;

export const capitalOneStatementFixture = `
  Capital One
  Statement Closing Date 01/02/2026
  Transactions Trans Date Post Date Description Amount
  12/03 12/04 NETFLIX.COM NETFLIX.COM CA 15.49
  12/09 12/10 WHOLEFDS SUGARLOAF GA 42.19
  12/27 12/28 AUTOPAY PAYMENT - THANK YOU -125.00
  Total Transactions
`;

export const amexStatementFixture = `
  American Express
  Closing Date 01/02/2026
  Date Description Amount
  12/05 WALMART SUPERCENTER LAWRENCEVILLE GA 28.44
  12/12 SPOTIFY USA NEW YORK NY 11.99
  12/29 PAYMENT RECEIVED - THANK YOU -80.00
  Fees
`;

export const synchronyStatementFixture = `
  amazon synchrony
  Previous Balance as of 01/24/2026 588.57
  Payments -588.57
  Other Credits -42.60
  Purchases/Debits + 322.12
  New Balance as of 02/20/2026 279.52
  Transaction Activity
  01/28 PAYMENT RECEIVED - THANK YOU -588.57
  02/02 AMAZON.COM 45.11
  02/07 WHOLE FOODS MARKET 61.28
  02/15 AMAZON MARKETPLACE 215.73
  Interest Charge Calculation
`;
